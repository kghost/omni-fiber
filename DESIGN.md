...
- **The Cooperative Wait Primitives**:
  ...
  - `WaitFor()`: Cooperatively yields until `_FinishedChildren` is not empty, pops the first completed child using `TryWait()`, and returns the `std::shared_ptr<Fiber>` child pointer.
