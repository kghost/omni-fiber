...
// from the _PendingSet by destroy SharedAwaitable to avoid being resumed when the event is triggered.
...
public:
  ...
  OMNIFIBER_API void AddFiberAwaitable(SharedAwaitable& awaitable);
  OMNIFIBER_API void RemoveFiberAwaitable(SharedAwaitable& awaitable);
  OMNIFIBER_API void Fire();

private:
  struct AddressCompare {
    bool operator()(std::reference_wrapper<SharedAwaitable> lhs, std::reference_wrapper<SharedAwaitable> rhs) const {
      return std::less<const SharedAwaitable*>{}(std::addressof(lhs.get()), std::addressof(rhs.get()));
    }
  };
  ...
  std::map<std::reference_wrapper<SharedAwaitable>, FiberAwaitableState, AddressCompare> _PendingSet;
};
